...
class SelectConsumer(override val config: Config)
  ...
  private val dbMetadata = system.actorOf(
    MySQLMetadataManager.props(clientInfo.host, clientInfo.port, clientInfo.user, Some(clientInfo.password)),
  ...
  override protected def decodeEvent(event: SelectEvent): Option[Event] = {
    ...
    getTable(event.database, event.table) match {
      case Some(table) ⇒
        ...
        Some(InsertMutation(table, rows, null))
      case None ⇒
